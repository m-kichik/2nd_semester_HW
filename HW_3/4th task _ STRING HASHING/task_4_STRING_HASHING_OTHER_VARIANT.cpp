...
void testHashFunctions(std::size_t N, std::size_t length) {
    std::ofstream outf("CollisionsInDifferentHashFunctions_Var2.txt");
    std::set <std::string> st = makeRandomWords(N, length);
    std::vector <std::string> randWords(st.begin(), st.end());

    std::vector <std::function<unsigned int(const char*, unsigned int)>> vectorOfHashFunctions = {{RSHash}, {JSHash}, {PJWHash},
